...
// Імпорт списку країн та функції витягування коду
...
export default function TransportTable({ 
  ...
  const filteredTransports = transports.filter(item => {
    ...
    // 1. Загальний пошук (назва транспорту, місто)
    ...
    // 2. Витягуємо лише код (наприклад "UA") з вибору користувача
    const codeFrom = extractCountryCode(searchFrom).toLowerCase();
    const codeTo = extractCountryCode(searchTo).toLowerCase();

    // 3. Шукаємо цей код у рядку маршруту (у дужках "(ua)" або як код)
  ...
  const formatLocation = (locStr) => {
    ...
    const codeMatch = locStr.match(/\(([A-Za-z]{2})\)/) || locStr.match(/([A-Za-z]{2})/);
    ...
    <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 p-3.5 sm:p-6">
      ...
      <div className="flex justify-between items-center mb-4 sm:mb-6">
        <h3 className="text-lg sm:text-xl font-bold text-slate-800 flex items-center gap-2.5 sm:gap-3">
          <span className="bg-blue-100 p-1.5 sm:p-2 rounded-xl text-blue-600">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-[11px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100">
              ...
              <th className="pb-4 px-3">Транспорт / Вага</th>
              <th className="pb-4 px-3">Дата</th>
              <th className="pb-4 px-3">Ставка</th>
                ...
                >
                  ...
                  <td className="py-4 px-3">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-slate-800">{item.vehicle || item.cargo}</span>
                      {item.weight && (
